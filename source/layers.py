...
class SynchRecursionBlock(RecursionBlock):
    def __init__(self, layers, steps=1, all_outputs=False, **kwargs):
        super().__init__(layers, steps=steps, all_outputs=all_outputs, **kwargs)

    def compute_output_shape(self, shape):
        # Adds an extra dimension after the batch
        shape = shape[:1] + [2] + shape[1:]

        # Calculates recursive ouput shapes
        shape = super().compute_output_shape(shape)
        return shape

    def call(self, in_data):
        data = [in_data]

        # Initializes recursive state to all zeros the size of the input
        recur = tf.zeros_like(in_data)

        # Loops through layers and appends to data
        for step in range(self.steps):
            # Resets data array to include only last element broadcasted
            # to the input shape of the first layer stacked with the input
            data = [tf.stack([in_data, recur], axis=1)]

            for index, layer in enumerate(self.layers):
                data.append(layer(data[index]))

            # Updates recur state to last data element
            recur = broadcast_pad(data[-1], in_data)
...
class Hebbian2(SimpleDense):
    ...
    def call(self, in_data):
        # Updates weights using loss gradient weighted by hebbian delta values

        # Gets loss gradient
        gradient = self.w - self.prev_weight

        # Updates weight with hebbian weighting
        self.w.assign_add(gradient * self.delta)
        ...
        self.delta.assign(tf.math.tanh(delta))
...
class Hebbian3(SimpleDense):
    ...
    def build(self, shape):
        ...
        # Hebbian values
        self.hebb_vals = self.add_weight("hebbian_vals", (2, shape[-1], self.units), 
                                    trainable=False, 
                                    initializer="zeros")
        
        # Inhibitory and excitatory weights
        self.hebb_weights = self.add_weight("hebbian_weights", (2, shape[-1], self.units), 
                                    trainable=False, 
                                    initializer="ones")

    def call(self, in_data):
        # Gets loss gradient and expands
        loss_grad = self.w - self.prev_weight
        expand_grad = tf.expand_dims(loss_grad, axis=0)

        hebbian = tf.reduce_sum(self.hebb_vals * self.hebb_weights, axis=0)
        delta = tf.math.sigmoid((loss_grad - hebbian)**2)

        # Weights gradient with delta value and appies to weight
        self.w.assign(self.prev_weight + loss_grad * delta)

        self.prev_weight.assign(self.w)

        # Updates inhibitory and excitatory weights and gets delta value
        weights = tf.math.divide_no_nan(expand_grad * tf.math.softmax(self.hebb_weights, axis=0), self.hebb_vals)
        self.hebb_weights.assign(weighted_update(self.hebb_weights, weights, self.rate))
        ...
        # Assigns hebbian values
        self.hebb_vals.assign(vals)
...
class Hebbian4(SimpleDense):
    ...
    def build(self, shape):
        ...
        # Input saving value
        self.prev_in = self.add_weight("prev_input", shape, 
                                    trainable=False, 
                                    initializer="zeros")

    def call(self, in_data):
        in_data = tf.math.tanh(in_data)
        # Gets loss gradient
        loss_grad = self.w - self.prev_weight

        # Gets hebbian values with previous input and new weight

        # Gets difference between new and old outputs
        out_diff = tf.matmul(self.prev_in, self.w) - tf.matmul(self.prev_in, self.prev_weight)
        
        # Adds dimensions to correspond axes
        in_expand = tf.expand_dims(self.prev_in, axis=-1)
        out_expand = tf.expand_dims(out_diff, axis=-2)
        excit = tf.reduce_mean(out_expand * in_expand, axis=0)

        # Gets hebbian inhibition based on correlation of weights to output
        out_correl = tf.reduce_sum(self.prev_weight * out_expand, axis=-1)
        out_correl = tf.expand_dims(out_correl, axis=-1)
        inhib = tf.reduce_mean(out_expand * out_correl, axis=0)

        # Gets weight delta step averaged over batch
        delta = tf.math.tanh(excit - inhib)

        # Updates weight with hebbian learning rate weighted by loss gradient reward
        self.w.assign(self.prev_weight + delta * loss_grad)

        # Saves current weight
        self.prev_weight.assign(self.w)

        # Saves current input
        self.prev_in.assign(in_data)
...
class HebbianDropout(SimpleDense):
    def __init__(self, units, use=0.5, **kwargs):
        super().__init__(units, **kwargs)
        self.use = tf.constant(use, dtype=tf.float32)
    ...
    def call(self, in_data):
        # Updates weights using loss gradient with some probabilty and hebbian with another
        # Gets binary mask for when to use hebbian
        hebb_mask = rand_bool_mask(self.use, shape=self.w.shape)

        # Gets loss gradient
        gradient = (self.w - self.prev_weight)

        # Weights hebbian values by gradient
        weighted_delta = gradient * self.delta

        # Gets step by using hebb and gradient where the mask says
        step = tf.where(hebb_mask, weighted_delta, gradient)

        # Updates weight
        self.w.assign(self.prev_weight + step)
        ...
        # Gets weight delta step averaged over batch
        delta = tf.reduce_mean(hebbian - inhib, axis=0)

        # Assigns delta step with tanh normalization
        self.delta.assign(tf.math.tanh(delta))
...
class NueralMemory(tf.keras.layers.Layer):
    def __init__(self, normalize=0.1, **kwargs):
        super().__init__(**kwargs)
    ...
    def build(self, in_shape):
        # Creates memory state
        self.memory = self.add_weight("memory", in_shape, trainable=False, initializer="zeros")

    def call(self, in_data):
        # Updates memory with weighted input
        self.memory.assign(weighted_update(self.memory, in_data, self.normalize))
